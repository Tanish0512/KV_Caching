...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not self.flash:
            print("WARNING: using slow attention(KV Cached1). Flash Attention requires PyTorch >= 2.0")
    ...
    def forward(self, x, past_kv=None):
        ...
        if past_kv is not None:
            ...
            k = torch.cat((past_k, k), dim=2) # Concatenate along Time dimension
        ...
        # ONLY apply mask if we are processing more than one token (the prompt phase)
        if T > 1:
            att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
        # If T=1, no mask is needed because we can't see the future anyway!
...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            logits, past_kv = self(idx_cond, past_kvs = past_kv)
